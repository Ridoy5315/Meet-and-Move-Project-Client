...
import { getUpcomingEvents } from "@/services/host/event";
...
const UpcomingEventsPage = async ({
  ...
  const upcomingEventsResult = await getUpcomingEvents(queryString);
  console.log("upcoming events result", upcomingEventsResult);
  const totalPages = Math.ceil(
    (upcomingEventsResult?.meta?.total || 1) / (upcomingEventsResult?.meta?.limit || 1),
  );
  return (
    <PageLayout
      header={
        <PageHeader title="My Upcoming Events" description="View and manage all your upcoming events scheduled on the platform." />
      }
    >
      <EventFilters />
